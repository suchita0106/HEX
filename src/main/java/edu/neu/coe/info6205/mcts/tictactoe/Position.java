...
public class Position {
    ...
    boolean threeInARow() {
        ...
        for (int x = 0; x < gridSize; x++) {
            ...
            int[] row = projectRow(x);
            int[] column = projectCol(x);

            int[] distinctRowValues = Arrays.stream(row).distinct().toArray();
            int[] distinctColValues = Arrays.stream(column).distinct().toArray();

            if (distinctRowValues.length == 1) {
                return true;
            } else if (distinctColValues.length == 1) {
                return true;
            }
        }
        return false;
